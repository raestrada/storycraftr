...
| **1.4** | **Design RAG Quality Measurement Tests** | Review `RAG-eval-report.md` to select a framework (e.g., RAGAS) and define metrics for faithfulness, context relevance, and answer relevancy.                                                                                                                                                                                                                                                                              | **Done** |
| **1.5** | **Evaluate and Refine RAG Retrieval**    | - [x] Implemented evaluation tests using the RAGAS framework. <br> - [x] Refined retrieval to use confidence scores instead of a fixed number of results. <br> - [x] Considered context limits and merged overlapping chunks to optimize context quality. <br> - [x] Added practical implementation examples to `docs/source/rag-evaluation.md` and `TESTS.md`.                                                            | **Done** |
...
| **2.2** | **Update Configuration**          | Modify `storycraftr/utils/core.py` (`load_book_config`) and the `storycraftr.json` structure to support multiple providers. Add fields for `api_base_url`, `api_key`, and `model_name` to allow easy switching to Mistral.ai or other providers.                                                                                                                                                                                                                                                                                                                                                                                                                                                     | **Done** |
| **2.3** | **Update CLI Commands**           | Refactor the `storycraftr chat` command (and any others that use the agent) to work with the new agent logic. This includes updating how the agent is initialized and how messages are sent.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | **Done** |
| **2.4** | **Fix Critical Bugs in Chat CLI** | Based on code review: <br> - **Exception Handling**: Replace broad `except Exception` with specific exceptions in `chat` command. <br> - **State Management**: Correct history update order. Append user input to history _before_ the API call.                                                                                                                                                                                                                                                                                                                                                                                                                                                     | **Done** |
| **2.5** | **Improve Chat Logic**            | Based on code review: <br> - **Idempotent Ingestion**: Make `ingest_book_data` check if data is already ingested. <br> - **Response Formatting**: Re-add prompt instruction for Markdown formatting. <br> - **RAG Validation**: Add a check to validate that RAG retrieval found relevant context.                                                                                                                                                                                                                                                                                                                                                                                                   | **Done** |
...
| Task ID | Description                      | Key Actions                                                                                                                                                                                                                                                                                                    | Status      |
| :------ | :------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------- |
| **3.1** | **End-to-End Testing**           | Manually test the full user workflow using the `storycraftr chat` command. Validate against: <br> - OpenAI's API. <br> - A Mistral.ai compatible endpoint. <br> Ensure that context from the book is correctly retrieved and used in responses.                                                                | In Progress |
| **3.2** | **Write Integration Tests**      | Add integration tests to the new test suite that cover the main chat workflow, mocking the API calls but testing the RAG pipeline and prompt construction.                                                                                                                                                     | **Done**    |
| **3.3** | **Address Code Review Feedback** | - **Resource Management**: Optimized `ingest_book_data()` to avoid instantiating embedding generator if data is already present. <br> - **Configurable Threshold**: Made RAG `distance_threshold` configurable in `storycraftr.json`. <br> - **Documentation**: Added comment explaining `distance_threshold`. | **Done**    |
| **3.4** | **Align Chunking Parameters**    | - Aligned `chunk_size` and `chunk_overlap` with OpenAI's file search defaults (800/400). <br> - Made chunking parameters configurable in `storycraftr.json`.                                                                                                                                                   | **Done**    |
| **3.5** | **Improve Ingestion Logic**      | - **Testing**: Added unit tests for idempotent ingestion. <br> - **Monitoring**: Added logging for skipped/performed ingestion. <br> - **Error Handling**: Improved error messages for ChromaDB exceptions.                                                                                                    | **Done**    |

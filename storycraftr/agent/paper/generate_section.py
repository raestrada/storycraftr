...
def _generate_section(
    ...
    if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
        ...
        content = refine_prompt_template.format(prompt=prompt, paper_title=paper_title)
    else:
        console.print(f"[yellow]Generating new {section_name}...[/yellow]")
        content = new_prompt_template.format(prompt=prompt, paper_title=paper_title)
    ...
    save_to_markdown(
        book_path, f"sections/{section_name}.md", section_name.title(), section_content
    )
...
def generate_results(book_path: str, prompt: str) -> str:
    ...
    return _generate_section(
        book_path, prompt, "results", RESULTS_PROMPT_NEW, RESULTS_PROMPT_REFINE
    )
...
def generate_discussion(book_path: str, prompt: str) -> str:
    ...
    return _generate_section(
        book_path, prompt, "discussion", DISCUSSION_PROMPT_NEW, DISCUSSION_PROMPT_REFINE
    )
...
def generate_conclusion(book_path: str, prompt: str) -> str:
    ...
    return _generate_section(
        book_path, prompt, "conclusion", CONCLUSION_PROMPT_NEW, CONCLUSION_PROMPT_REFINE
    )


def generate_custom_section(
    book_path: str, prompt: str, section_title: str, order: int
) -> str:
    """
    ...
    refine_prompt = CUSTOM_SECTION_PROMPT_REFINE.format(
        section_title=section_title, prompt=prompt
    )

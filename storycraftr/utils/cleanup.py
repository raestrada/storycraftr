...
def cleanup_vector_stores(book_path: str):
    ...
    for vector_store in vector_stores.data:
        try:
            ...
            console.print(f"[bold blue]Deleting vector store {vector_store.id}...[/bold blue]")
            vector_stores_api.delete(vector_store_id=vector_store.id)
            
        except Exception as e:
            console.print(f"[bold red]Error deleting vector store {vector_store.id}: {str(e)}[/bold red]")
            continue

    console.print("[bold green]All vector stores and files have been deleted.[/bold green]") 
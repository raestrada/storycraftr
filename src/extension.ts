...
function checkPythonDependencies(): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        exec('python3 --version', (error, stdout, stderr) => {
            if (error) {
                vscode.window.showErrorMessage('Python is not installed. Please install Python.');
            ...
            exec('pipx --version', (error, stdout, stderr) => {
                if (error) {
                    vscode.window.showWarningMessage('pipx not installed, installing pipx...');
                    // Install pipx
...
function checkStoryCraftr(resolve: () => void, reject: (reason: any) => void): void {
    exec('pipx list', (error, stdout, stderr) => {
        if (!stdout.includes('storycraftr')) {
            vscode.window.showWarningMessage('StoryCraftr not installed, installing StoryCraftr...');
            exec('pipx install storycraftr', (error, stdout, stderr) => {
                ...
                vscode.window.showInformationMessage('StoryCraftr installed successfully.');
                resolve();
            });
        } else {
            resolve();
...
export function activate(context: vscode.ExtensionContext): void {
    let disposable = vscode.commands.registerCommand('storycraftr.start', () => {
        ...
        if (workspaceFolders) {
            ...
            const storyCraftrConfigPath = path.join(workspaceFolder, 'storycraftr.json');
            ...
            if (fs.existsSync(storyCraftrConfigPath)) {
                ...
                vscode.window.showErrorMessage('StoryCraftr is not initialized. Please ensure storycraftr.json exists in the project.');
            }
        } else {

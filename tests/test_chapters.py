...
def test_generate_chapter(
    ...
    mock_exists.assert_called_once_with(
        os.path.join("my_book", "chapters", "chapter-1.md")
    )
    ...
    mock_save.assert_called_once_with(
        "my_book", "chapter-1.md", "Chapter 1", "Generated Chapter Content"
    )
...
def test_generate_cover(
    mock_save, mock_load_book_config, mock_assistant, mock_thread, mock_message
):
    # Mocks
    ...
    mock_message.assert_called_once_with(
        thread_id="thread_id", content=expected_prompt, assistant="assistant_object"
    )
    mock_save.assert_called_once_with(
        "my_book", "cover.md", "Cover", "Generated Cover Content"
    )
...
def test_generate_back_cover(
    ...
    mock_save.assert_called_once_with(
        "my_book", "back_cover.md", "Back Cover", "Generated Back Cover Content"
    )
...
def test_generate_epilogue(
    ...
    # Assertions
    mock_exists.assert_called_once_with(
        os.path.join("my_book", "chapters", "epilogue.md")
    )
    mock_message.assert_called_once_with(
        thread_id="thread_id",
    ...
    mock_save.assert_called_once_with(
        "my_book", "epilogue.md", "Epilogue", "Generated Epilogue Content"
    )
